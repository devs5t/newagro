...
const InvestCard: React.FC<InvestCardProps> = ({
  ...
  return (
    <div
      className={`flex flex-col md:flex-row  w-full rounded-lg bg-lightblue/[.15] py-8 px-5 shadow relative grid grid-cols-1 md:grid-cols-2 ${containerClasses}`}
      ...
      {open && (
        <>
          ...
          <div className={"grid grid-cols-2 gap-2 mt-5 w-full mt-5"}>
            ...
            />
            <Button
              text={`${t('investment.retire')} ${token}`}
              extraClasses="px-2 md:px-0 w-full border-2 border-blue font-bold text-blue py-2 px-0"
            />
            <Button
